		...
		mDrawer.setOnDrawerStateChangeListener(new OnDrawerStateChangeListener() {
			...
			public void onDrawerStateChange(int oldState, int newState) {
				if (oldState == 1) {
					...
					actionbar_menu.setColorFilter(getResources().getColor(R.color.buttonHover), PorterDuff.Mode.SRC_ATOP);
				}
...
//		sharingIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, ""+AppSettings.getInstance().get_group_app_id_OnlyID());
		...
		ss = ss.replace("(groupID)", AppSettings.getInstance().get_group_app_id_OnlyID());
